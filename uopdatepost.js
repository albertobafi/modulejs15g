...
window.addEventListener('load', () => {
    ...
    console.log(idPost)
    fetch(
        `https://genjs-3e4eb-default-rtdb.firebaseio.com/posts/${idPost}.json`,
        {
            method: 'GET',
        }
    )
...
const updateUser =  (objPost, idPost) => {
    const xhttp = new XMLHttpRequest()
    xhttp.open( "PATCH" , `https://genjs-3e4eb-default-rtdb.firebaseio.com/posts/${idPost}.json`, true)
    xhttp.onload = function(data) {
        if(data.target.status === 200){
...
delete__post.addEventListener('click', () => {
    let idPost = location.search.slice(8)
    fetch (`ttps://genjs-3e4eb-default-rtdb.firebaseio.com/posts/${idPost}.json`, {
        method: 'DELETE'
    })
    .then(response =>{
        response
